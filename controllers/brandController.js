...
exports.updateBrandStatus = async (req, res) => {
  try {
    ...
    if (status === true || status === 'true') status = 'Publish';
    if (status === false || status === 'false') status = 'Hold';

    // Normalize string casing (publish → Publish, HOLD → Hold)
    if (typeof status === 'string') {
      status = status.charAt(0).toUpperCase() + status.slice(1).toLowerCase();
    }

    // If status is still not valid, just toggle current brand status
    const brand = await Brand.findById(req.params.brandId);
    ...
    if (!['Publish', 'Hold'].includes(status)) {
      // Auto-toggle if frontend sends nothing valid
      status = brand.status === 'Publish' ? 'Hold' : 'Publish';
    }
